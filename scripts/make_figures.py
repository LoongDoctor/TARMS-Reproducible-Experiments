...
from tarms_experiments.deterministic_figures import (  # noqa: E402
...
def latest_aamos_run(results: Path, *, submission: bool) -> Path:
    candidates = sorted(
        (results / "processed/aamos").glob("aamos-*"),
        reverse=True,
    )
    for candidate in candidates:
        ...
        if (
            not submission
            or manifest.environment.get("profile") == "submission"
        ):
            return candidate
    profile = "submission-profile " if submission else ""
    raise ValueError(f"no {profile}AAMOS analysis run found")
...
def main() -> int:
    ...
    parser.add_argument(
        "--figure",
        choices=("python", "fabric", "late", "aamos", "component", "window"),
        required=True,
    )
    ...
    parser.add_argument("--aamos-run", type=Path)
    parser.add_argument("--component-run", type=Path)
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()

    submission = args.mode == "submission"
    if submission and args.output is None:
        parser.error("submission mode requires explicit --output")
    ...
    if args.figure == "python":
    ...
    elif args.figure == "aamos":
        run = args.aamos_run or latest_aamos_run(
            args.results, submission=submission
        )
        ...
        outputs = render_window_tradeoff_figure(
